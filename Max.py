...
def main():
    ...
    if page == "Sentiment Analysis":
        ...
        if user_text and analytics.sentiment_model is not None:
            ...
            fig = px.bar(prob_chart_df, x="Sentiment", y="Probability", title="Prediction Probabilities", text=prob_chart_df["Probability"].round(2))
            ...
            try:
                ...
                if not data.empty:
                    ...
                    fig = px.scatter(data, x=x_axis, y=y_axis,
                                   color='Segment', title='Customer Segments', opacity=0.7)
                    st.plotly_chart(fig, use_container_width=True)
        ...
        if uploaded_file:
            try:
                ...
                if 'Date' in data.columns:
                    ...
                    # Sales Trend
                    fig1 = px.line(filtered_data, x='Date', y='Sales',
                                title='📈 Sales Trend Over Time')
                    st.plotly_chart(fig1, use_container_width=True)

                    # Top Products (if column exists)
                    if 'Product' in filtered_data.columns:
                        top_products = filtered_data.groupby('Product')['Sales'].sum().sort_values(ascending=False).head(10)
                        fig2 = px.bar(top_products, x=top_products.index, y=top_products.values,
                                      title="🏆 Top 10 Products by Sales")
                        st.plotly_chart(fig2, use_container_width=True)

                    # Regional Sales (if column exists)
